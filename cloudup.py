...
def uploadText():
    ...
    if(len(myWords) >= 3):
        googleResp = requests.get('https://www.googleapis.com/customsearch/v1?key='+gKey+'&cx=015528857462049392512:iqcemyfrksa&q='+myWords[0]+'+'+myWords[1]+'+'+myWords[2]+'+wine')
    elif(len(myWords) < 3 and len(myWords) >= 2):
        googleResp = requests.get('https://www.googleapis.com/customsearch/v1?key='+gKey+'&cx=015528857462049392512:iqcemyfrksa&q='+myWords[0]+'+'+myWords[1]+'+wine')
    else:
        googleResp = requests.get('https://www.googleapis.com/customsearch/v1?key='+gKey+'&cx=015528857462049392512:iqcemyfrksa&q='+myWords[0]+'+wine')
